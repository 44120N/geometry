...
function calcCircumference(r) {
    ...
    let circumferenceResult = `\\(C = 2 × ${pi} × ${r} = ${parseFloat((2*Math.PI*r).toFixed(floatAccuration))}\\)`;
    resultCircumference.innerHTML = circumferenceResult;
    setTimeout(function () {
...
function calcRadius_Circumference(C) {
    resultCircumference.classList.add("form__equation");
    let circumferenceResult = `\\(r = \\frac{${C}}{2 × ${pi}} = ${parseFloat((C/(2*Math.PI)).toFixed(floatAccuration))}\\)`;
    resultCircumference.innerHTML = circumferenceResult;
    setTimeout(function () {
        MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultCircumference]);
    }, 0);
