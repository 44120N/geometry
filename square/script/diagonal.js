...
function calcDiagonal(a) {
  ...
  let diagonalResult = `\\(d = ${a} \\sqrt{2} = ${parseFloat(
    (a * Math.sqrt(2)).toFixed(floatAccuration)
  )}\\)`;
  resultDiagonal.innerHTML = diagonalResult;
  setTimeout(function () {
...
function calcSide_Diagonal(d) {
  resultDiagonal.classList.add("form__equation");
  let diagonalResult = `\\(a = \\frac{${d}}{\\sqrt{2}} = ${parseFloat(
    (d / Math.sqrt(2)).toFixed(floatAccuration)
  )}\\)`;
  resultDiagonal.innerHTML = diagonalResult;
  setTimeout(function () {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultDiagonal]);
