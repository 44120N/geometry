...
function calcDiagonal(l, w) {
    ...
    let diagonalResult = `\\(d = \\sqrt{${l}^2 + ${w}^2} = \\sqrt{${l**2} + ${w**2}} = \\sqrt{${l**2+w**2}} = ${Math.sqrt(l**2 + w**2)}\\)`
    resultDiagonal.innerHTML = diagonalResult;
    setTimeout(function () {
...
function calcLength_diagonal(d, w) {
        resultDiagonal.classList.add("form__equation");
        let diagonalResult = `\\(l = \\sqrt{${d}^2 - ${w}^2} = \\sqrt{${d**2} - ${w**2}} = \\sqrt{${d**2-w**2}} = ${Math.sqrt(d**2 - w**2)}\\)`;
        resultDiagonal.innerHTML = diagonalResult;
        setTimeout(function () {
            MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultDiagonal]);
        }, 0);
...
function calcWidth_diagonal(d, l) {
    resultDiagonal.classList.add("form__equation");
    let diagonalResult = `\\(w = \\sqrt{${d}^2 - ${l}^2} = \\sqrt{${d**2} - ${l**2}} = \\sqrt{${d**2-l**2}} = ${Math.sqrt(d**2 - l**2)}\\)`;
    resultDiagonal.innerHTML = diagonalResult;
    setTimeout(function () {
        MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultDiagonal]);
    }, 0);
