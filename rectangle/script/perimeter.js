...
function calcLength_Perimeter(P, w) {
        resultPerimeter.classList.add("form__equation");
        let perimeterResult = `\\(l = \\frac{${P} - 2 × ${w}}{2} = \\frac{${P}-${2*w}}{2} = \\frac{${P-2*w}}{2} = ${parseFloat(((P - 2*w)/2).toFixed(floatAccuration))}\\)`;
        resultPerimeter.innerHTML = perimeterResult;
        setTimeout(function () {
            MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultPerimeter]);
        }, 0);
...
function calcWidth_Perimeter(P, l) {
    resultPerimeter.classList.add("form__equation");
    let perimeterResult = `\\(w = (${P} - 2 × ${l})/2 = ${parseFloat((P - 2*l )/2).toFixed(floatAccuration)}\\)`;
    resultPerimeter.innerHTML = perimeterResult;
    setTimeout(function () {
        MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultPerimeter]);
    }, 0);
