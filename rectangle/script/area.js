...
function calcArea(l, w) {
    ...
    let areaResult = `\\(A = ${l} × ${w} = ${parseFloat((l * w).toFixed(floatAccuration))}\\)`;
    resultArea.innerHTML = areaResult;
    setTimeout(function () {
...
function calcLength(A, w) {
    resultArea.classList.add("form__equation");
    let areaResult = `\\(l = \\frac{${A}}{${w}} = ${parseFloat((A/w).toFixed(floatAccuration))}\\)`;
    resultArea.innerHTML = areaResult;
    setTimeout(function () {
        MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultArea]);
    }, 0);
...
function calcWidth(A, l) {
    resultArea.classList.add("form__equation");
    let areaResult = `\\(h = \\frac{${A}}{${l}} = ${parseFloat((A/l).toFixed(floatAccuration))}\\)`;
    resultArea.innerHTML = areaResult;
    setTimeout(function () {
        MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultArea]);
    }, 0);
