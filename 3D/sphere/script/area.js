...
function calcArea(r) {
  ...
  let areaResult = `\\(A = ${pi} × ${r}^2 = ${pi} × ${r ** 2} = ${
    pi * r ** 2
  } = ${parseFloat((Math.PI * r ** 2).toFixed(floatAccuration))}\\)`;
  resultArea.innerHTML = areaResult;
  setTimeout(function () {
...
function calcRadius_Area(A) {
  resultArea.classList.add("form__equation");
  let areaResult = `\\(r = \\sqrt{\\frac{${A}}{${pi}}} = \\sqrt{${
    A / pi
  }} = ${parseFloat(Math.sqrt(A / Math.PI).toFixed(floatAccuration))}\\)`;
  resultArea.innerHTML = areaResult;
  setTimeout(function () {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultArea]);
  }, 0);
