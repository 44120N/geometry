...
function callArea() {
  ...
  let b = parseFloat(document.getElementById("bA").value);
  ...
  if (
    ...
    if (a <= 0) {
      ...
      result.push("a should be positive number");
    }
    if (a > maxValue) {
      resultArea.classList.add("red");
      result.push("a: Number limit exceeded");
    }
    if (b <= 0) {
      resultArea.classList.add("red");
      result.push("b should be positive number");
    }
    if (b > maxValue) {
      resultArea.classList.add("red");
      result.push("b: Number limit exceeded");
    }
    if (h <= 0) {
      resultArea.classList.add("red");
...
function calcArea(b, h) {
  ...
  let areaResult = `\\(A = \\frac{1}{2} × ${b} × ${h} = ${parseFloat(
    (0.5 * b * h).toFixed(floatAccuration)
  )}\\)`;
  resultArea.innerHTML = areaResult;
  setTimeout(function () {
...
function calcBase(A, h) {
  resultArea.classList.add("form__equation");
  let areaResult = `\\(b = \\frac{2*${A}}{${h}} × = ${parseFloat(
    ((2 * A) / h).toFixed(floatAccuration)
  )}\\)`;
  resultArea.innerHTML = areaResult;
  setTimeout(function () {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultArea]);
...
function calcHeight(A, b) {
  resultArea.classList.add("form__equation");
  let areaResult = `\\(h = \\frac{2*${A}}{${b}} × = ${parseFloat(
    ((2 * A) / b).toFixed(floatAccuration)
  )}\\)`;
  resultArea.innerHTML = areaResult;
  setTimeout(function () {
    MathJax.Hub.Queue(["Typeset", MathJax.Hub, resultArea]);
